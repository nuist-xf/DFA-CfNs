...
class Period_encoder_decoder_predication(tf.keras.Model):
    def __init__(self, seq_in_len, seq_out_len, enc_in, period_len):
        ...
        self.conv1d = layers.Conv1D(filters=1, kernel_size=kernel_size, strides=1,
                                    padding='same', use_bias=False)
        self.linear1 = layers.Dense(self.seg_num_y)
        self.linear2 = layers.Dense(self.period_len)

    def call(self, x):
        # x: [batch_size, enc_in, seq_len]
        batch_size = tf.shape(x)[0]
        x = tf.transpose(x, perm=[0, 2, 1])
        x_reshape = tf.reshape(x, [-1, self.seq_len, 1])
        conv_output = self.conv1d(x_reshape)
        conv_output = tf.reshape(conv_output, [-1, self.enc_in, self.seq_len])
        x = conv_output + x
        x = tf.reshape(x, [-1, self.seg_num_x, self.period_len])
        x = tf.transpose(x, perm=[0, 2, 1])
        y = self.linear1(x)
        y = tf.transpose(y, perm=[0, 2, 1])
        y = self.linear2(y)
        y = tf.transpose(y, perm=[0, 2, 1])
        y = tf.reshape(y, [batch_size, self.enc_in, self.pred_len])
        return y

class Multi_period_predication(tf.keras.Model):
    def __init__(self, seq_in_len, seq_out_len, enc_in, period_channels, device='cuda'):
        ...
        self.device = device
        self.period_encoders = [
            Period_encoder_decoder_predication(
                seq_in_len, seq_out_len, enc_in, period_len if not hasattr(period_len, 'item') else period_len.item())
            for period_len in period_channels
        ]
        self.period_weights = tf.Variable(tf.ones([len(period_channels), 1, 1]), trainable=True)

    def call(self, x):
        # x: [batch_size, enc_in, seq_len]
        batch_size = tf.shape(x)[0]
        period_outputs = []
        for period_encoder in self.period_encoders:
            period_outputs.append(period_encoder(x))
        period_outputs = tf.stack(period_outputs, axis=1)  # [batch_size, num_periods, enc_in, pred_len]
        weighted_output = tf.reduce_sum(period_outputs * self.period_weights, axis=1)  # [batch_size, enc_in, pred_len]
...
class CfcCell(tf.keras.layers.Layer):
    def __init__(self, units, hparams, **kwargs):
        ...
        self.units = units
        self.state_size = [units, units]  # [hidden_state, sum_state]
        ...
        self.built = True
        # 新增 self.memory 和 self.sum 用于记录历史信息
        self.memory = []
        # self.sum = tf.Variable(tf.zeros((self.batch_size, self.units)), trainable=False)
        self.sum = None  # 不初始化固定形状
    ...
    def get_initial_state(self, inputs=None, batch_size=None, dtype=None):
        return [
            tf.zeros((batch_size, self.units), dtype=dtype),  # hidden_state
            tf.zeros((batch_size, self.units), dtype=dtype)  # sum_state
        ]

    def call(self, inputs, states, **kwargs):
        hidden_state, sum_state = states
        t = 1.0
        ...
        # 时间间隔处理
        ts_new = 1 / t
        w_ts = tf.where(ts_new > 0, tf.exp(ts_new * (1 - 2 * tf.math.log(ts_new))), 0)
        ...
        if self._minimal:
            ...
            new_sum_state = sum_state + tf.abs(ff1)
            new_hidden = (
                -self.A
                * tf.exp(- t * tf.abs(self.w_tau) - w_ts * new_sum_state)
                * ff1
                + self.A
            )
        else:
            # Cfc
            ff1 = tf.nn.tanh(self.ff1(x))
            ff2 = tf.nn.tanh(self.ff2(x))
            t_a = self.time_a(x)
            t_b = self.time_b(x)
            t_sum = -t_a * w_ts + t_b
            self.memory.append(t_sum)
            new_sum_state = sum_state + t_sum
            t_interp = tf.nn.sigmoid(new_sum_state)
            if self._no_gate:
                new_hidden = ff1 + t_interp * ff2
            else:
                new_hidden = ff1 * (1.0 - t_interp) + t_interp * ff2

        return new_hidden, [new_hidden, new_sum_state]
...
class MixedCfcCell(tf.keras.layers.Layer):
    ...
    def call(self, inputs, states, **kwargs):
        ...
        sum_state = tf.zeros_like(ode_state)  # 或根据实际形状初始化
        ode_output, new_ode_state = self.cfc([ode_input, elapsed], [ode_state, sum_state])       # ode_output, new_ode_state = self.ctrnn.call([ode_input, elapsed], [ode_input])
