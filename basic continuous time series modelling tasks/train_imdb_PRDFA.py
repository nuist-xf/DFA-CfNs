...
import numpy as np
import argparse
from tf_cfc_PRDFA import CfcCell, MixedCfcCell, LTCCell, Multi_period_predication
...
def eval(config, index_arg, verbose=1):
    ...
    # 添加 Multi_period_predication 层
    period_channels = [5]  # 这里可以根据需要调整
    multi_period_layer = Multi_period_predication(
        seq_in_len=maxlen,
        seq_out_len=maxlen,
        enc_in=config["embed_dim"],
        period_channels=period_channels
    )
    multi_period_output = multi_period_layer(cell_input)
    ...
    output_states = rnn(multi_period_output)
