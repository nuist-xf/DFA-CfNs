...
def show_pred_final(all_y_true, all_predict_values, horizon,save_name):
    time_steps = horizon
    ...
    plt.legend()
    plt.savefig(f'./output/{save_name}/assets/the first month pred electricity.png')
    # plt.show()
    ...
    plt.plot(predict_values_extracted[node_id], linewidth=1.0, label='pred')
    plt.legend()
    plt.savefig(f'./output/{save_name}/assets/the first month pred cooling.png')
    # plt.show()
    ...
    plt.plot(predict_values_extracted[node_id], linewidth=1.0, label='pred')
    plt.legend()
    plt.savefig(f'./output/{save_name}/assets/the first month pred heating.png')
    # plt.show()
...
# save_name=''
# all_y_true_loaded_np = np.load(f'./实验数据保存/{save_name}/result/all_y_true.npy')
# all_predict_value_loaded_np = np.load(f'./实验数据保存/{save_name}/result/all_predict_value.npy')
# show_pred_final(all_y_true_loaded_np, all_predict_value_loaded_np, 24,save_name)
