...
class Cfc(nn.Module):
    def __init__(
        ...
        # self.in_seq = hparams['in_seq']
        # self.in_f = hparams['in_f']

        self.minimal = hparams["minimal"]
        if use_ltc:
    ...
    def forward(self, x1, x2 , timespans=None, mask=None):
        # print(x.shape)
        ...
        if timespans is not None:
            timespans_sum = torch.sum(timespans, dim=-1)
        else:
        ...
        readout = readout + seq_mean[:, :, 0].unsqueeze(1)
        return readout
